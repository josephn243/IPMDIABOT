module.exports = {
    ...
    async execute(message, args, Discord, client) {
        ...
        let embed = new Discord.MessageEmbed()
            .setColor('#09215C')
            .setTitle( `${user.nickname} has started an AFK check for The Void`)
            .setThumbnail('https://i.imgur.com/7JGSvMq.png')
            .setDescription('React appropriately below')
            .addFields(
                {name: 'To Participate', value: `${Void}`,inline: true},
                {name: 'Run Requirements', value: `${vial}`, inline: true},
                {name: 'Classses', value: `${knight} ${warrior} ${paladin}${trickster}${mystic}`},
                {name: 'Status Effects', value: `${slow} ${armorbreak} ${expose} ${mseal} ${fungaltome}`, inline:true},
                {name: 'Consumables', value: `${dexeff} ${atteff}`, inline: true}
