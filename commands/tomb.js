module.exports = {
    ...
    async execute(message, args, Discord, client) {
        ...
        let embed = new Discord.MessageEmbed()
            .setColor('#EDFF00')
            .setTitle( `${user.nickname} has started an AFK check for Tomb of the Ancients`)
            .setThumbnail('https://i.imgur.com/6VuYOsy.png')
            .setDescription('React appropriately below')
            .addFields(
                {name: 'To Participate', value: `${tomb}`,inline: true},
                {name: 'Classses', value: `${knight} ${warrior} ${paladin} ${trickster} ${mystic}`},
                {name: 'Status Effects', value: ` ${paralyze} ${slow} ${daze} ${armorbreak} ${expose} ${mseal} ${fungaltome}  `, inline:true},
