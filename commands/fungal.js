module.exports = {
    ...
    async execute(message, args, Discord, client) {
        ...
        let embed = new Discord.MessageEmbed()
            .setColor('#BA41CE')
            .setTitle( `${user.nickname} has started an AFK check for Fungal & Crystal Caverns`)
            .setThumbnail('https://i.imgur.com/kC1mFqy.png')
            .setDescription('React appropriately below')
            .addFields(
                {name: 'To Participate', value: `${fungal}`,inline: true},
                {name: 'Classses', value: `${knight} ${warrior} ${paladin}${trickster}${mystic}`},
                {name: 'Status Effects', value: ` ${slow} ${daze} ${armorbreak} ${expose} ${mseal} ${fungaltome}  `, inline:true},
