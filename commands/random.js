module.exports = {
    ...
    async execute(message, args, Discord, client) {
        ...
        let embed = new Discord.MessageEmbed()
            .setColor('#A0470D')
            .setTitle( `${user.nickname} has started an AFK check for random dungeons`)
            .setThumbnail('https://static.drips.pw/rotmg/wiki/Environment/Portals/Guild%20Explanation%20Portal.png')
            .setDescription('React appropriately below')
            .addFields(
                {name: 'To Participate', value: `${itsduckinglit}`,inline: true},
                {name: 'Classses', value: `${knight} ${warrior} ${paladin}${trickster}${mystic}`},
                {name: 'Status Effects', value: ` ${slow} ${paralyze} ${daze} ${armorbreak} ${expose} ${mseal} ${fungaltome}  `, inline:true},
